#!/bin/bash
# Discover native OS service versions and status on RHEL-family systems.
# Checks sshd, crond, rsyslogd, systemd-journald.
...
# Cron (crond on RHEL)
CRON_VER=$(rpm -q cronie --qf '%{VERSION}' 2>/dev/null || echo "unknown")
CRON_STATUS=$(systemctl is-active crond 2>/dev/null || echo "unknown")
