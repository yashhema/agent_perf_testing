...
class LabORM(Base):
    ...
    secret_management_properties: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True
    )
    ...
    con_properties: Mapped[list["LabConPropertiesORM"]] = relationship(
        "LabConPropertiesORM", back_populates="lab", lazy="selectin"
    )
    package_preferences: Mapped[list["LabPackagePreferencesORM"]] = relationship(
        "LabPackagePreferencesORM", back_populates="lab", lazy="selectin"
    )
...
class ServerORM(Base):
    ...
    winrm_username: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    ...
    __table_args__ = (
        ...
        Index("idx_servers_type", "server_type"),
    )
...
class BaselineORM(Base):
    ...
    Types: vsphere_snapshot, vm_image, ami, managed_state.
    """
    ...
    # Provider reference (snapshot UUID / image ID / policy ID)
    ...
    baseline_conf: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
...
class TestRunTargetORM(Base):
    ...
    calibration_json: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
...
class TestRunExecutionORM(Base):
    ...
    __table_args__ = (
        Index(
            "idx_test_run_execution_active",
            "test_run_id",
            postgresql_where=(
                Column("status").notin_(["ended", "ended_error", "abandoned"])
            ),
        ),
...
class CalibrationResultORM(Base):
    ...
    calibration_run_id: Mapped[Optional[UUID]] = mapped_column(
    ...
    __table_args__ = (
        ...
        Index("idx_calibration_lookup", "target_id", "baseline_id", "loadprofile"),
    )
...
class ExecutionWorkflowStateORM(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    test_run_execution_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("test_run_execution.id"), nullable=False
    )
    target_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("servers.id"), nullable=False
    )
    ...
    base_package_lst: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    # Measured results: [{package_id, install_status, measured_version, matched, ...}]
    base_package_lst_measured: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    ...
    # =========================================================================
    initial_package_lst: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    initial_package_lst_measured: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    ...
    # =========================================================================
    upgrade_package_lst: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    upgrade_package_lst_measured: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    ...
    jmeter_package_lst: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    # Measured results: [{package_id, install_status, measured_version, matched, ...}]
    jmeter_package_lst_measured: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)
    ...
    error_history: Mapped[list] = mapped_column(JSONB, default=list, nullable=False)
...
class HardwareProfileORM(Base):
    ...
    # Relationships
    scenarios: Mapped[list["ScenarioORM"]] = relationship(
        "ScenarioORM", back_populates="hardware_profile"
    )
    targets: Mapped[list["TargetORM"]] = relationship(
        "TargetORM", back_populates="hardware_profile"
    )
...
class ScenarioORM(Base):
    ...
    hardware_profile: Mapped[Optional["HardwareProfileORM"]] = relationship(
        "HardwareProfileORM", back_populates="scenarios"
    )
    baseline: Mapped["BaselineORM"] = relationship("BaselineORM")
...
class PackageORM(Base):
    ...
    # Delivery configuration (typed JSONB - supports SCRIPT, INTUNE, JAMF, WORKFLOW)
    delivery_config: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
...
class AgentORM(Base):
    ...
    lab_con_properties: Mapped[list["LabConPropertiesORM"]] = relationship(
        "LabConPropertiesORM", back_populates="agent"
    )
    lab_package_preferences: Mapped[list["LabPackagePreferencesORM"]] = relationship(
        "LabPackagePreferencesORM", back_populates="agent"
    )
...
class EnvironmentORM(Base):
    ...
    targets: Mapped[list["TargetORM"]] = relationship(
        "TargetORM", back_populates="environment"
    )
...
class TargetORM(Base):
    """ORM model for targets table.

    Represents a device under test (server, endpoint, VM, etc.).
    Distinct from ServerORM which is for load generators and infrastructure.
    """
    ...
    # OS Information
    os_family: Mapped[str] = mapped_column(String(20), nullable=False)
    os_vendor: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    os_major: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    os_minor: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    hardware_profile_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("hardware_profiles.id"), nullable=True
    )

    # Labels for roles (emulator_a, emulator_b, loadgen, etc.)
    ...
    # Relationships
    environment: Mapped["EnvironmentORM"] = relationship(
        "EnvironmentORM", back_populates="targets"
    )
    hardware_profile: Mapped[Optional["HardwareProfileORM"]] = relationship(
        "HardwareProfileORM", back_populates="targets"
    )

    __table_args__ = (
        Index("idx_targets_env", "env_id"),
...
class LabConPropertiesORM(Base):
    """ORM model for lab_con_properties table.

    Stores connection configurations for each lab.
    Supports multiple connection types: MFOO, TANIUM, NFS, SSM, INTUNE, etc.
    """

    __tablename__ = "lab_con_properties"
    ...
    # Optional agent-specific connection
    agent_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("agents.id"), nullable=True
    )

    # Connection properties (url, username, password, client_id, etc.)
    con_properties: Mapped[dict] = mapped_column(JSONB, nullable=False)
    ...
    # Relationships
    lab: Mapped["LabORM"] = relationship("LabORM", back_populates="con_properties")
    agent: Mapped[Optional["AgentORM"]] = relationship(
        "AgentORM", back_populates="lab_con_properties"
    )

    __table_args__ = (
        Index("idx_lab_con_properties_lab", "lab_id"),
        Index("idx_lab_con_properties_type", "con_type"),
    )


class LabPackagePreferencesORM(Base):
    """ORM model for lab_package_preferences table.

    Defines package delivery preferences per lab.
    Specifies preferred and fallback connection types for each package type.
    """

    __tablename__ = "lab_package_preferences"
    ...
    # Optional agent-specific preference
    agent_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("agents.id"), nullable=True
    )

    # Optional OS-specific preference
    os_vendor_family: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)

    # Connection type preferences
    preferred_con_type: Mapped[str] = mapped_column(String(50), nullable=False)
    fallback_con_type: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    ...
    # Relationships
    lab: Mapped["LabORM"] = relationship(
        "LabORM", back_populates="package_preferences"
    )
    agent: Mapped[Optional["AgentORM"]] = relationship(
        "AgentORM", back_populates="lab_package_preferences"
    )

    __table_args__ = (
        Index("idx_lab_pkg_pref_lab", "lab_id"),
        Index("idx_lab_pkg_pref_type", "package_type"),
    )
...
class TestRunExecutionScenarioStatusORM(Base):
    ...
    calibration_data_json: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
