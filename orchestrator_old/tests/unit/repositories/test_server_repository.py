...
class TestServerRepository:
    ...
    async def test_create_linux_loadgen(
        ...
        assert server.server_type == ServerRole.LOAD_GENERATOR
    ...
    async def test_get_by_type_app_server(
        ...
        result = await repo.get_by_type(lab_id, ServerRole.APP_SERVER)

        assert len(result) == 1
        assert result[0].server_type == ServerRole.APP_SERVER
    ...
    async def test_get_by_type_load_generator(
        ...
        result = await repo.get_by_type(lab_id, ServerRole.LOAD_GENERATOR)

        assert len(result) == 1
        assert result[0].server_type == ServerRole.LOAD_GENERATOR
    ...
    async def test_get_by_type_active_only(
        ...
        result = await repo.get_by_type(lab_id, ServerRole.APP_SERVER, active_only=True)
    ...
    async def test_get_by_type_include_inactive(
        ...
        result = await repo.get_by_type(
            lab_id, ServerRole.APP_SERVER, active_only=False
        )
